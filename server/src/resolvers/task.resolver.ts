...
export const taskResolvers = {
  ...
  Mutation: {
    createTask: async (
      _: unknown,
      { input }: { input: { controlId: string; ownerId: string; dueDate: Date; description: string; notes?: string } }
    ) => {
    ...
    updateTask: async (
      _: unknown,
      { input }: { input: { id: string; controlId: string; ownerId: string; dueDate: Date; description: string; notes?: string } }
    ) => {
      const [control, owner] = await Promise.all([
        Control.findById(input.controlId),
        Owner.findById(input.ownerId),
      ...
      const task = await Task.findByIdAndUpdate(
        input.id,
        { control: input.controlId, owner: input.ownerId, dueDate: input.dueDate, description: input.description, notes: input.notes },
        { new: true }
      ).exec();
      if (!task) {throw new GraphQLError(`Task ${input.id} not found`);}
      return task;
    },
